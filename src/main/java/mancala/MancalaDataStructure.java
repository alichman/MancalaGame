...
public class MancalaDataStructure implements Serializable{
    ...
    private static final long serialVersionUID = -5697930686818069344L;
    ...
    public MancalaDataStructure(int startStones){
        ...
        data.add(new Store());
        System.out.println("BIG TEST: " +getNumStones(7));
    }
    ...
    public int getNumStones(){
        int value;
        if(iteratorPos ==  storePos(1) || iteratorPos == storePos(2)){
            value = getStoreCount(iteratorPos/7);
        } else{
            value = getNumStones(iteratorPos);
        }
        return value;
    }
    ...
    private int storePos(int playerNum) {
        if(playerNum <1 || playerNum > 2){
            throw new RuntimeException("Invalid Player Position: " + playerNum);
        }
    ...
    /* Helper method to get side of current pit */
    int getCurrentSide(){
        return (iteratorPos+1)/7 + 1;
    }

    /* Helper method to get current pit number */
    int getCurrentPosition(){
        return iteratorPos;
    }

    void cleanUp(){
        setIterator(12, 2, false);
        next();
        while(iteratorPos != 13){
            Countable pit = next();
            System.out.println("side: " + getCurrentSide() + " num: " + (getCurrentPosition()+1));
            addToStore(getCurrentSide(), pit.removeStones());
        }
    }
